...
template <std::size_t N>
using select_uint_least_t = decltype(
    select_uint_least(std::integral_constant<std::size_t, bit_weight(N)>()));
...
constexpr void cswap(std::tuple<Tys...> &a,
                     std::tuple<Tys...> &b,
...
constexpr Iterator partition(Iterator left,
                             Iterator right,
        ...
        if (compare_(*it, value_)) {
            ...
            return doitfirst(
                ...
                std::integral_constant<
                    bool, next_highest_power_of_two(next) - 1 == next>{});
        } else
